  ...
  return events.map((event) => {
    ...
    switch (event.type) {
      ...
      case "PullRequestEvent":
        title = `${displayLogin} ${event.payload.action} pull request ${wrapPR(
      ...
      case "RepositoryEvent":
        title = `${displayLogin} ${event.payload.action} repository ${wrapRepo(
          event.repo
        )}`;
        content = repoToContent(event)
        break;
      case "StatusEvent":
        title = `${displayLogin} set ${
          event.payload.state
        } status on commit ${event.sha.slice(0, 7)} in ${wrapRepo(event.repo)}`;
      ...
      case "WatchEvent":
        ...
        break;
      default:
        title = `${event.type} event occurred in ${wrapRepo(event.repo)}`;
        content = repoToContent(event)
        break;
    }
